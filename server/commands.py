...
def ooc_cmd_bg(client, arg):
    ...
    try:
        if client.is_mod or client.is_cm:
...
def ooc_cmd_help(client, arg):
    if len(arg) != 0:
        raise ArgumentError('This command has no arguments.')
    help_url = 'https://github.com/Jerm125067/TSU3-FOR-DRO'
...
def ooc_cmd_gm(client, arg):
    ...
    client.server.broadcast_globalooc(client, arg, True)
...
def ooc_cmd_rpmode(p_client, arg):
    if not p_client.server.config['rp_mode_enabled']:
        ...
        return
    if not p_client.is_mod and not p_client.is_cm:
        raise ClientError('You must be authorized to do that.')
    if len(arg) == 0:
